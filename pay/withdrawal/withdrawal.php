<?php
class Pay_Withdrawal extends Pay_Checkout {
  ...
  public function confirm($token="") {
    $mtoken = trim($token);
    $result = $this->utility->httpGetRequest($this->setup->getWithdrawalConfirmUrl()."?withdrawalToken=" . $mtoken);
    if(count($result) > 0) {
      switch ($result['status']) {
        case 'completed':
          $this->status = $result['status'];
            $this->response_text = "Withdrawal status is ".strtoupper($result['status']);
    ...
    public function check($key, $value) {
        //echo $this->setup->getWithdrawalCheckUrl()."?customdataKey=" . urlencode($key) . "&customdataValue=" . urlencode($value) . "<br><br>";
        $result = $this->utility->httpGetRequest($this->setup->getWithdrawalCheckUrl()."?customdataKey=" . urlencode($key) . "&customdataValue=" . urlencode($value));
        return $result;
    }
